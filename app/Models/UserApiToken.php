...
class UserApiToken extends Model
{
    ...
    public static function chkSocailUser($data)
    {
        $query = self::select('user_api_token.*')
            ->selectRaw('platform_type,platform_id')
            ->join('users AS u','u.id','=','user_api_token.user_id')
            ->where('email',$data['email'])
            //->where('platform_id',$data['platform_type'])
            ->first();
        return $query;
    }

    public static function checkSocialUser($platform_id)
    {
        $query = self::join('users AS u','u.id','=','user_api_token.user_id')
                    ->select('u.*')
