...
class Notification extends Model
{
     ...
     */
    protected $fillable = [
        'unique_id', 'identifier', 'actor_id','actor_slug', 'actor_type', 'target_id', 'target_type', 'reference_id', 'reference_module',
    ...
    public static function getUserNotifications($params, $unique_id = 0)
    {
        ...
        $query = self::select('notification.*')
                        ->selectRaw("u.name AS actor_name, u.email AS actor_email, u.driver AS actor_driver, u.image_url  AS actor_image_url")
    ...
    public static function sendPushNotification($identifier,$notification_data,$customData = [], $device_type = 'android')
    {
        ...
        $customData['actor_id'] = $notification_data['actor']->id;
        ...
        foreach($target_users as $notifyUser)
        {
            $notification_buld_data[] = [
                ...
                'actor_slug'                 => $notification_data['actor']->slug,
