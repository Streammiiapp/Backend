...
class CmsModule extends Model
{
    ...
    {
        $modules = [];
        $authUser = CustomHelper::currentUser();
        if( !empty($authUser) ){
            $authUser = $authUser->toArray();
            if( $authUser['cms_role']['is_super_admin'] )
                $modules = self::getSuperAdminModules();
            else
                $modules = self::getSubAdminModules($authUser['cms_role_id']);
        }
        return $modules;
    }
    ...
    {
        $query = self::with(['child' => function($query){
                    $query->where('status',1)->orderBy('sort_order','asc');
                }])
                ->where('parent_id',0)
                ->where('status','1')
    ...
    public static function getCurrentRoutePrivilege()
    {
        ...
        $authUser = CustomHelper::currentUser()->toArray();
        $query = self::join('cms_module_permissions AS cmp','cmp.cms_module_id','=','cms_modules.id')
                        ...
                        ->where('cmp.cms_role_id',$authUser['cms_role']['id'])
                        ->first();
        return $query;
    }
}
