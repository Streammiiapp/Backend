...
class CRUDCrontroller extends Controller
{
    ...
    {
        //check permission
        if( CustomHelper::currentUser()->cms_role_id != 1 && CustomHelper::modulePermission()->is_view != 1)
    ...
    {
        //check permission
        if( CustomHelper::currentUser()->cms_role_id != 1 && CustomHelper::modulePermission()->is_add != 1)
            return redirect()->route('admin.dashboard')->with('error',__('app.permission_denied'));
    ...
    public function store()
    {
        ...
        return redirect()
            ->route($current_route[0] . '.edit',[ str_replace('-','_',$current_route[0]) => $record->slug ])
    ...
    {
        //check permission
        if( CustomHelper::currentUser()->cms_role_id != 1 && CustomHelper::modulePermission()->is_update != 1)
            return redirect()->route('admin.dashboard')->with('error',__('app.permission_denied'));
    ...
    public function show($slug)
    {
        ...
        if(method_exists($this,'beforeRenderDetailView')) {
            $response = $this->beforeRenderDetailView($this->__data['record']);
            if ($this->__is_error) {
                return $response;
            }
        }
    ...
    {
        //check permission
        if( CustomHelper::currentUser()->cms_role_id != 1 && CustomHelper::modulePermission()->is_delete != 1){
