...
class CmsUserController extends CRUDCrontroller
{
        ...
        switch ($action){
            case 'POST':
                $validator = Validator::make($this->__request->all(), [
                    ...
                    'email'            => 'required|email|max:100|unique:cms_users,email,NULL,deleted_at',
                    'image_url'        => 'required|image',
                    'mobile_no'        => [
                        'required',
                        'unique:cms_users,mobile_no,NULL,deleted_at',
                        'regex:/^(\+?\d{1,3}[-])\d{9,11}$/'
                    ],
                    'password'         => [
                        'required',
                    ...
                    'cms_role_id'      => 'required|exists:cms_roles,id,deleted_at,NULL',
                ],$custom_messages);
                break;
            case 'PUT':
                $validator = Validator::make($this->__request->all(), [
                    ...
                    'email'   => [
                        ...
                        'email',
                        'max:100',
                        Rule::unique('cms_users')->whereNull('deleted_at')->ignore($slug,'slug')
                    ],
                    'mobile_no' => [
                        'required',
                        'regex:/^(\+?\d{1,3}[-])\d{9,11}$/',
    ...
    {
        $this->__data['getCmsRole'] = CmsRole::getCmsRole();
    }
    ...
    private function _submitProfile($request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'email'     => [
                'required',
                'email',
                Rule::unique('cms_users')->whereNull('deleted_at')->ignore(CustomHelper::currentUser()->id),
            ],
            'mobile_no' => [
                'min:8',
                'max:15',
        ...
        CmsUser::updateProfile($request->all());
    ...
    private function _submitChangePassword($request)
    {
        ...
        if( !Hash::check($request['current_password'],CustomHelper::currentUser()->password) )
            return redirect()->back()->with('error','Invalid current password');

        CmsUser::updatePassword(CustomHelper::currentUser()->id,$request['new_password']);
    ...
    public function logout()
    {
        ...
        return redirect()->route('admin.login',['auth_token' => config('constants.admin_auth_token')]);    }
}
